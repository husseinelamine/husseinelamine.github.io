...
function init() {
    ...
    function handleWheel(e) {
        ico.position.z += e.deltaY * 0.0001;
        ico.position.z = Math.min(Math.max(ico.position.z, -5), 0);
        camera.position.y = Math.min(Math.max(camera.position.y - e.deltaY * 0.0001, -1.5), 1);
...
function onWindowResize() {
    ...
    // change fov for mobile devices
    camera.fov = window.innerWidth < 600 ? 75 : 50;
    camera.updateProjectionMatrix();
